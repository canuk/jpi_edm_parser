...
import { JpiEdmParser, HeaderParseError, ChecksumError } from './parser.js';
...
async function parseFile(file) {
  try {
    ...
    if (e instanceof HeaderParseError || e instanceof ChecksumError) {
      ...
      showError(`Error parsing file: ${e.message || String(e)}`);
    }
  }
}
...
function createFlightRow(flight, tailNumber) {
...
function downloadFlightCsv(flight, tailNumber) {
  const csv = flight.toCsv();
  const filename = generateFilename(tailNumber, flight);
  downloadFile(csv, filename, 'text/csv');
}

function generateFilename(tailNumber, flight) {
...
async function downloadAllAsZip() {
  ...
  try {
    ...
    showError(`Error creating ZIP: ${e.message || String(e)}`);
  } finally {
